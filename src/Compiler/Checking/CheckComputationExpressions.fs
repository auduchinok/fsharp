...
let TcComputationExpression (cenv: cenv) env (overallTy: OverallTy) tpenv (mWhole, interpExpr: Expr, builderTy, comp: SynExpr) =
    ...
    let customOperationMethods =
        ...
        |> List.choose (fun methInfo ->
            if not (IsMethInfoAccessible cenv.amap mBuilderVal ad methInfo) then
                None
            else
                let nameSearch =
    ...
    let customOperationMethodsIndexedByKeyword =
        if cenv.g.langVersion.SupportsFeature LanguageFeature.OverloadsForCustomOperations then
            ...
            |> Seq.map (fun (nm, group) -> (nm, group |> Seq.toList))
        else
            customOperationMethods
            |> Seq.groupBy (fun (nm, _, _, _, _, _, _, _, _) -> nm)
    ...
    let hasCustomOperations () =
        if isNil customOperationMethods then
            CustomOperationsMode.Denied
        else
            CustomOperationsMode.Allowed

    let isCustomOperation nm =
        tryGetDataForCustomOperation nm |> Option.isSome
            ...
            if isLikeGroupJoin then
                Some(
                    FSComp.SR.customOperationTextLikeGroupJoin (
                        nm.idText,
                        customOperationJoinConditionWord nm,
                        customOperationJoinConditionWord nm
                    )
                )
                ...
                eNameResEnv =
                    ...
                    ||> Seq.fold (fun nenv (nm, _, _, _, _, _, _, _, methInfo) ->
                        AddFakeNameToNameEnv
                            nm
                            nenv
                            (Item.CustomOperation(nm, (fun () -> customOpUsageText (ident (nm, mBuilderVal))), Some methInfo)))
            }
                ...
                if cenv.g.langVersion.SupportsFeature LanguageFeature.OverloadsForCustomOperations then
                    ...
                    |> List.exists (fun info ->
                        ...
                        | Some args ->
                            args
                            |> List.exists
                                (fun (ParamAttribs(isParamArrayArg, _isInArg, isOutArg, optArgInfo, _callerInfo, _reflArgInfo)) ->
    ...
    let (|OnExpr|_|) nm synExpr =
        ...
        | Some _ ->
            ...
            | SynExpr.App(funcExpr = SynExpr.App(funcExpr = e1; argExpr = SingleIdent opName); argExpr = e2) when
                opName.idText = customOperationJoinConditionWord nm
                ->
    ...
    let MatchOnExprOrRecover alreadyGivenError nm (onExpr: SynExpr) =
        match onExpr with
        | OnExpr nm (innerSource, SynExprParen(keySelectors, _, _, _)) -> (innerSource, keySelectors)
        | _ ->
            if not alreadyGivenError then
    ...
    let (|JoinOp|_|) synExpr =
        JoinOrGroupJoinOp customOperationIsLikeJoin synExpr

    let (|GroupJoinOp|_|) synExpr =
        JoinOrGroupJoinOp customOperationIsLikeGroupJoin synExpr
    ...
    let (|GroupJoinExpr|_|) synExpr =
        ...
        | InExpr(GroupJoinOp(nm, innerSourcePat, _, alreadyGivenError), intoExpr, mGroupJoinCore) ->
            let onExpr, intoPat, alreadyGivenError =
                MatchIntoSuffixOrRecover alreadyGivenError nm intoExpr
    ...
    let (|ForEachThenJoinOrGroupJoinOrZipClause|_|) strict synExpr =
        ...
        | ForEachThen(isFromSource,
                      firstSourcePat,
                      firstSource,
                      JoinOrGroupJoinOrZipClause(nm, secondSourcePat, secondSource, keySelectorsOpt, pat3opt, mOpCore),
                      innerComp) when
            ...
            let intoInfoOpt =
                ...
                | Some(intoWordRange, intoInfo) ->
                    ...
                    CallNameResolutionSink
                        cenv.tcSink
                        (intoWordRange, env.NameEnv, item, emptyTyparInst, ItemOccurence.Use, env.eAccessRights)
    ...
    let checkForBinaryApp comp =
        ...
        | StripApps(SingleIdent nm, [ StripApps(SingleIdent nm2, args); arg2 ]) when
            ...
            && (match tryExpectedArgCountForCustomOperator nm2 with
                | Some n -> n > 0
                | _ -> false)
    ...
    let rec tryTrans
        ...
        <| fun () ->
            ...
            | ForEachThenJoinOrGroupJoinOrZipClause true (isFromSource,
                                                          firstSourcePat,
                                                          firstSource,
                                                          nm,
                                                          secondSourcePat,
            ...
            | SynExpr.ForEach(spFor, spIn, SeqExprOnly _seqExprOnly, isFromSource, pat, sourceExpr, innerComp, _mEntireForEach) ->
                ...
                if
                    isNil (TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env mFor ad "For" builderTy)
                then
                ...
                let varSpace =
                    addVarsToVarSpace varSpace (fun _mCustomOp env ->
                        ...
                        let _, _, vspecs, envinner, _ =
                            TcMatchPattern cenv (NewInferenceType g) env tpenv pat None
            ...
            | SynExpr.While(spWhile, guardExpr, innerComp, _) ->
                ...
                if
                    isNil (
                        TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env mWhile ad "While" builderTy
                    )
                then
                ...
                if
                    isNil (
                        TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env mWhile ad "Delay" builderTy
                    )
                then
            ...
            | SynExpr.TryFinally(innerComp, unwindExpr, _mTryToLast, spTry, spFinally, trivia) ->
                ...
                if
                    isNil (
                        TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env mTry ad "TryFinally" builderTy
                    )
                then
                    error (Error(FSComp.SR.tcRequireBuilderMethod ("TryFinally"), mTry))

                if
                    isNil (TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env mTry ad "Delay" builderTy)
                then
            ...
            | SynExpr.Sequential(sp, true, innerComp1, innerComp2, m, _) ->
                ...
                if isQuery && checkForBinaryApp innerComp1 then
                    ...
                    | Some c ->
                        ...
                        if
                            isNil (
                                TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env m ad "Delay" builderTy
                            )
                        then
            ...
            | SynExpr.LetOrUse(
                ...
                if
                    isNil (TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env mBind ad "Using" builderTy)
                then
            ...
            | SynExpr.LetOrUseBang(
                ...
                if
                    isNil (TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env mBind ad "Bind" builderTy)
                then
                ...
                if hasBindReturnN && Option.isSome (convertSimpleReturnToExpr varSpace innerComp) then
                    ...
                    let varSpace =
                        addVarsToVarSpace varSpace (fun _mCustomOp env ->
                            ...
                            let _, _, vspecs, envinner, _ =
                                TcMatchPattern cenv (NewInferenceType g) env tpenv consumePat None
            ...
            | SynExpr.TryWith(innerComp, clauses, mTryToLast, spTry, spWith, trivia) ->
                ...
                if
                    isNil (
                        TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env mTry ad "TryWith" builderTy
                    )
                then
                ...
                let dataCompAfterOp =
                    ...
                    | StripApps(SingleIdent nm, args) ->
                        ...
                        if argCountsMatch then
                            ...
                            errorR (
                                Error(
                                    FSComp.SR.tcCustomOperationHasIncorrectArgCount (nm.idText, expectedArgCount, args.Length),
                                    nm.idRange
                                )
                            )
    ...
    and transBind q varSpace bindRange addBindDebugPoint bindName bindArgs (consumePat: SynPat) (innerComp: SynExpr) translatedCtxt =
        ...
        | Some(innerExpr, customOpInfo) when
            (let bindName = bindName + "Return"
             ...
             not (
                 isNil (
                     TryFindIntrinsicOrExtensionMethInfo ResultCollectionSettings.AtMostOneResult cenv env bindRange ad bindName builderTy
                 )
             ))
            ->
    ...
    let lambdaExpr, tpenv =
        TcExpr cenv (MustEqual(mkFunTy g builderTy overallTy)) env tpenv lambdaExpr
