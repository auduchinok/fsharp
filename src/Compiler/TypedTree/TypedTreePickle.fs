    ...
    static member Create(mkEmpty, lnk, isLinked, nm, n) =
        {
            ...
            IsLinked = isLinked
            Name = nm
            Nodes = Array.init n (fun _i -> mkEmpty ())
        }
...
type ReaderState =
    {
        ...
        /// One EntityRef wrapper per inlerefs row. The rows are already shared, but every mention used to
...
let u_bytes st = (u_byte_memory st).ToArray()
...
let u_osgn_ref (inMap: NodeInTable<_, _>) st =
    ...
    if n < 0 || n >= inMap.Count then
        ufailwith st ("u_osgn_ref: out of range, table = " + inMap.Name + ", n = " + string n)
...
let lookup_uniq st tbl n =
    ...
    if n < 0 || n >= arr.Length then
        ufailwith
            st
            ("lookup_uniq in table "
             + tbl.itbl_name
             + " out of range, n = "
             + string n
             + ", sizeof(tab) = "
             + string (Array.length arr))
...
let p_hole () =
    ...
    (fun x st ->
        match h with
        | Some f -> f x st
        | None -> pfailwith st "p_hole: unfilled hole")
...
let u_list_core f n st = List.init n (fun _ -> f st)
...
// A huge number of these occur in pickled F# data, so make them unique
let decode_nleref st ccuTab stringTab (a, b) =
    mkNonLocalEntityRef (lookup_ccuref st ccuTab a) (Array.map (lookup_string st stringTab) b)
...
let decode_simpletyp st _ccuTab _stringTab nlerefTab a =
...
let p_simpletyp x st =
    p_int (encode_simpletyp st.occus st.ostrings st.onlerefs st.osimpletys st.oscope x) st
...
let unpickleObjWithDanglingCcus
    ...
    let pubpathTab =
        new_itbl "ipubpaths" (Array.map (decode_pubpath st2 stringTab) pubpathTab)

    let nlerefTab =
        new_itbl "inlerefs" (Array.map (decode_nleref st2 ccuTab stringTab) nlerefTab)

    let simpletypTab =
        new_itbl "simpleTyTab" (Array.map (decode_simpletyp st2 ccuTab stringTab nlerefTab) simpleTyTab)
        ...
        let st1 =
            {
                ...
                ientities =
                    NodeInTable<_, _>
                        .Create(Tycon.NewUnlinked, (fun osgn tg -> osgn.Link tg), (fun osgn -> osgn.IsLinked), "itycons", ntycons)
                itypars =
                    NodeInTable<_, _>
                ...
                ianoninfos =
                    NodeInTable<_, _>
                        .Create(
                            AnonRecdTypeInfo.NewUnlinked,
                            (fun osgn tg -> osgn.Link tg),
                            (fun osgn -> osgn.IsLinked),
                            "ianoninfos",
                            nanoninfos
                        )
                istrings = stringTab
...
let p_ILVersion (x: ILVersionInfo) st =
    p_tup4 p_uint16 p_uint16 p_uint16 p_uint16 (x.Major, x.Minor, x.Build, x.Revision) st
...
let p_ILAssemblyRef (x: ILAssemblyRef) st =
    ...
    p_tup6
        p_string
        (p_option p_bytes)
        (p_option p_ILPublicKey)
        p_bool
        (p_option p_ILVersion)
        (p_option p_string)
        (x.Name, x.Hash, x.PublicKey, x.Retargetable, x.Version, x.Locale)
        st
...
let p_ILScopeRef x st =
    ...
    | ILScopeRef.Module mref ->
        p_byte 1 st
        p_ILModuleRef mref st
    | ILScopeRef.Assembly aref ->
        p_byte 2 st
        p_ILAssemblyRef aref st
    // Encode primary assembly as a normal assembly ref
    | ILScopeRef.PrimaryAssembly ->
        p_byte 2 st
        p_ILAssemblyRef st.oglobals.ilg.primaryAssemblyRef st
...
let u_ILAssemblyRef st =
    ...
    | 0 ->
        let a, b, c, d, e, f =
            u_tup6 u_string (u_option u_bytes) (u_option u_ILPublicKey) u_bool (u_option u_ILVersion) (u_option u_string) st
...
let p_ILArrayShape =
    p_wrap (fun (ILArrayShape x) -> x) (p_list (p_tup2 (p_option p_int32) (p_option p_int32)))
...
let rec p_ILType ty st =
    ...
    | ILType.Array(shape, ty) ->
        p_byte 1 st
        p_tup2 p_ILArrayShape p_ILType (shape, ty) st
    | ILType.Value tspec ->
        p_byte 2 st
        p_ILTypeSpec tspec st
    | ILType.Boxed tspec ->
        p_byte 3 st
        p_ILTypeSpec tspec st
    | ILType.Ptr ty ->
    ...
    | ILType.Modified(req, tref, ty) ->
        p_byte 8 st
        p_tup3 p_bool p_ILTypeRef p_ILType (req, tref, ty) st
...
and p_ILTypeSpec (a: ILTypeSpec) st =
    p_tup2 p_ILTypeRef p_ILTypes (a.TypeRef, a.GenericArgs) st
...
let p_ILMethodRef (x: ILMethodRef) st =
    p_tup6
        ...
        p_ILTypes
        p_ILType
        (x.DeclaringTypeRef, x.CallingConv, x.GenericArity, x.Name, x.ArgTypes, x.ReturnType)
        st

let p_ILFieldRef (x: ILFieldRef) st =
    p_tup3 p_ILTypeRef p_string p_ILType (x.DeclaringTypeRef, x.Name, x.Type) st

let p_ILMethodSpec (x: ILMethodSpec) st =
    p_tup3 p_ILMethodRef p_ILType p_ILTypes (x.MethodRef, x.DeclaringType, x.GenericArgs) st

let p_ILFieldSpec (x: ILFieldSpec) st =
    p_tup2 p_ILFieldRef p_ILType (x.FieldRef, x.DeclaringType) st
...
let u_ILVolatility st =
    (match u_int st with
     | 0 -> Volatile
     | 1 -> Nonvolatile
     | _ -> ufailwith st "u_ILVolatility")
...
let decoders =
    [
        ...
        itag_ldelema,
        u_tup3 u_ILReadonly u_ILArrayShape u_ILType
        >> (fun (a, b, c) -> I_ldelema(a, false, b, c))
...
let decode_tab =
    let tab =
        Array.init 256 (fun n -> (fun st -> ufailwith st ("no decoder for instruction " + string n)))
...
let p_ILInstr x st =
    ...
    | AI_conv a ->
        ...
        p_ILBasicType a st
    | AI_conv_ovf a ->
        p_byte itag_conv_ovf st
        p_ILBasicType a st
    | AI_conv_ovf_un a ->
        p_byte itag_conv_ovf_un st
        p_ILBasicType a st
    | I_ldfld(Aligned, b, c) ->
        p_byte itag_ldfld st
        p_tup2 p_ILVolatility p_ILFieldSpec (b, c) st
    | I_ldsfld(a, b) ->
        p_byte itag_ldsfld st
        p_tup2 p_ILVolatility p_ILFieldSpec (a, b) st
    | I_stfld(Aligned, b, c) ->
        p_byte itag_stfld st
        p_tup2 p_ILVolatility p_ILFieldSpec (b, c) st
    | I_stsfld(a, b) ->
        p_byte itag_stsfld st
        p_tup2 p_ILVolatility p_ILFieldSpec (a, b) st
    | I_ldflda c ->
        p_byte itag_ldflda st
        p_ILFieldSpec c st
    | I_ldsflda a ->
    ...
    | I_ldtoken(ILToken.ILType ty) ->
        ...
        p_ILType ty st
    | I_ldstr s ->
        p_byte itag_ldstr st
        p_string s st
    | I_box ty ->
        p_byte itag_box st
        p_ILType ty st
    | I_unbox ty ->
        p_byte itag_unbox st
        p_ILType ty st
    | I_unbox_any ty ->
        p_byte itag_unbox_any st
        p_ILType ty st
    | I_newarr(a, b) ->
        p_byte itag_newarr st
        p_tup2 p_ILArrayShape p_ILType (a, b) st
    | I_stelem_any(a, b) ->
        p_byte itag_stelem_any st
        p_tup2 p_ILArrayShape p_ILType (a, b) st
    | I_ldelem_any(a, b) ->
        p_byte itag_ldelem_any st
        p_tup2 p_ILArrayShape p_ILType (a, b) st
    | I_ldelema(a, _, b, c) ->
        p_byte itag_ldelema st
        p_tup3 p_ILReadonly p_ILArrayShape p_ILType (a, b, c) st
    | I_castclass ty ->
        p_byte itag_castclass st
        p_ILType ty st
    | I_isinst ty ->
        p_byte itag_isinst st
        p_ILType ty st
    | I_ldobj(Aligned, Nonvolatile, c) ->
        p_byte itag_ldobj st
        p_ILType c st
    | I_stobj(Aligned, Nonvolatile, c) ->
        p_byte itag_stobj st
        p_ILType c st
    | I_sizeof ty ->
        p_byte itag_sizeof st
        p_ILType ty st
    | EI_ldlen_multi(n, m) ->
        p_byte itag_ldlen_multi st
        p_tup2 p_int32 p_int32 (n, m) st
    | EI_ilzero a ->
        p_byte itag_ilzero st
        p_ILType a st
    | I_initobj c ->
        p_byte itag_initobj st
        p_ILType c st
    | I_cpobj c ->
        p_byte itag_cpobj st
        p_ILType c st
...
let p_pos (x: pos) st =
    p_tup2 p_int p_int (x.Line, x.Column) st
...
let p_ident (x: Ident) st =
    p_tup2 p_string p_range (x.idText, x.idRange) st

let p_xmldoc (doc: XmlDoc) st =
...
let p_ucref (UnionCaseRef(a, b)) st =
    p_tup2 (p_tcref "ucref") p_string (a, b) st

let p_rfref (RecdFieldRef(a, b)) st =
    p_tup2 (p_tcref "rfref") p_string (a, b) st
...
let checkForInRefStructThisArg st ty =
    ...
    isFunTy g tauTy
    && isFunTy g (rangeOfFunTy g tauTy)
    && isInByrefTy g (domainOfFunTy g tauTy)
...
let rec p_vref ctxt x st =
    ...
    | VRefLocal x ->
        p_byte 0 st
        p_local_item_ref ctxt st.ovals x st
    | VRefNonLocal x ->
        p_byte 1 st
        p_nonlocal_val_ref x st
...
let p_MemberFlags (x: SynMemberFlags) st =
    p_tup6
        ...
        p_bool
        p_member_kind
        (x.IsInstance, false (* _x3UnusedBoolInFormat *) , x.IsDispatchSlot, x.IsOverrideOrExplicitImpl, x.IsFinal, x.MemberKind)
        st

let u_MemberFlags st : SynMemberFlags =
    let x2, _x3UnusedBoolInFormat, x4, x5, x6, x7 =
        u_tup6 u_bool u_bool u_bool u_bool u_bool u_member_kind st
...
let p_trait_sln sln st =
    ...
    | ILMethSln(a, b, c, d, None) ->
        p_byte 0 st
        p_tup4 p_ty (p_option p_ILTypeRef) p_ILMethodRef p_tys (a, b, c, d) st
    | FSMethSln(a, b, c, None) ->
        p_byte 1 st
        p_tup3 p_ty (p_vref "trait") p_tys (a, b, c) st
    | BuiltInSln -> p_byte 2 st
    | ClosedExprSln expr ->
        p_byte 3 st
        p_expr_fwd expr st
    | FSRecdFieldSln(a, b, c) ->
        p_byte 4 st
        p_tup3 p_tys p_rfref p_bool (a, b, c) st
    | FSAnonRecdFieldSln(a, b, c) ->
        p_byte 5 st
        p_tup3 p_anonInfo p_tys p_int (a, b, c) st
    | ILMethSln(a, b, c, d, Some e) ->
        p_byte 6 st
        p_tup5 p_ty (p_option p_ILTypeRef) p_ILMethodRef p_tys p_ty (a, b, c, d, e) st
...
let p_measure_con tcref st =
    p_byte 0 st
    p_tcref "measure" tcref st

let p_measure_var v st =
    p_byte 3 st
    p_tpref v st
...
let rec p_measure_power unt q st =
    if q = ZeroRational then
        p_measure_one st
    elif GetDenominator q = 1 then
...
let rec p_normalized_measure unt st =
    ...
    | Measure.Inv x ->
        p_byte 1 st
        p_normalized_measure x st
    | Measure.Prod(measure1 = x1; measure2 = x2) ->
        p_byte 2 st
        p_normalized_measure x1 st
        p_normalized_measure x2 st
    | Measure.Var v -> p_measure_var v st
...
let p_tyar_constraint x st =
    ...
    | TyparConstraint.AllowsRefStruct _ -> failwith $"%A{x} constraints should only be emitted to streamB"
...
let p_tyar_constraints cxs st =
    let cxs1, cxs2 =
        ...
        |> List.partition (function
            | TyparConstraint.NotSupportsNull _
            | TyparConstraint.AllowsRefStruct _ -> false
            | _ -> true)
...
let u_tyar_constraint st =
    ...
    | 9 ->
        u_tup2 u_ty u_ty st
        |> (fun (a, b) _ -> TyparConstraint.IsDelegate(a, b, range0))
...
let p_tyar_spec_data (x: Typar) st =
    p_tup5
        ...
        p_tyar_constraints
        p_xmldoc
        (x.typar_id, x.Attribs, int64 x.typar_flags.PickledBits, x.Constraints, x.XmlDoc)
        st
...
let p_ranges x st = p_option (p_tup2 p_range p_range) x st
...
and p_entity_spec_data (x: Entity) st =
    ...
    p_int64
        (x.entity_flags.PickledBits
         ||| (if flagBit then
                  EntityFlags.ReservedBitForPickleFormatTyconReprFlag
              else
                  0L))
        st
...
and p_entity_spec x st =
    p_osgn_decl st.oentities p_entity_spec_data x st
...
and p_member_info (x: ValMemberInfo) st =
    p_tup4
        ...
        (p_list p_slotsig)
        p_bool
        (x.ApparentEnclosingEntity, x.MemberFlags, x.ImplementedSlotSigs, x.IsImplemented)
        st
...
and u_attrib st : Attrib =
    let a, b, c, d, e, f =
        u_tup6 u_tcref u_attribkind (u_list u_attrib_expr) (u_list u_attrib_arg) u_bool u_dummy_range st
...
and p_const x st =
    ...
    | Const.Decimal s ->
        p_byte 17 st
        p_array p_int32 (System.Decimal.GetBits s) st
...
and p_dtree x st =
    ...
    | TDSwitch(a, b, c, d) ->
        p_byte 0 st
        p_tup4 p_expr (p_list p_dtree_case) (p_option p_dtree) p_dummy_range (a, b, c, d) st
    | TDSuccess(a, b) ->
        p_byte 1 st
        p_tup2 p_Exprs p_int (a, b) st
    | TDBind(a, b) ->
        p_byte 2 st
        p_tup2 p_bind p_dtree (a, b) st

and p_dtree_case (TCase(a, b)) st =
    p_tup2 p_dtree_discrim p_dtree (a, b) st

and p_dtree_discrim x st =
    match x with
    | DecisionTreeTest.UnionCase(ucref, tinst) ->
        p_byte 0 st
        p_tup2 p_ucref p_tys (ucref, tinst) st
...
and u_dtree st =
    ...
    | 0 ->
        let a, b, c, d =
            u_tup4 u_expr (u_list u_dtree_case) (u_option u_dtree) u_dummy_range st
...
and p_expr expr st =
    ...
    | Expr.Const(x, m, ty) ->
        p_byte 0 st
        p_tup3 p_const p_dummy_range p_ty (x, m, ty) st
    | Expr.Val(a, b, m) ->
        p_byte 1 st
        p_tup3 (p_vref "val") p_vrefFlags p_dummy_range (a, b, m) st
    | Expr.Op(a, b, c, d) ->
        p_byte 2 st
        p_tup4 p_op p_tys p_Exprs p_dummy_range (a, b, c, d) st
    ...
    | Expr.Lambda(_, a1, b0, b1, c, d, e) ->
        p_byte 4 st
        p_tup6 (p_option p_Val) (p_option p_Val) p_Vals p_expr p_dummy_range p_ty (a1, b0, b1, c, d, e) st
    | Expr.TyLambda(_, b, c, d, e) ->
        p_byte 5 st
        p_tup4 p_tyar_specs p_expr p_dummy_range p_ty (b, c, d, e) st
    | Expr.App(a1, a2, b, c, d) ->
        p_byte 6 st
        p_tup5 p_expr p_ty p_tys p_Exprs p_dummy_range (a1, a2, b, c, d) st
    | Expr.LetRec(a, b, c, _) ->
        p_byte 7 st
        p_tup3 p_binds p_expr p_dummy_range (a, b, c) st
    | Expr.Let(a, b, c, _) ->
        p_byte 8 st
        p_tup3 p_bind p_expr p_dummy_range (a, b, c) st
    | Expr.Match(_, a, b, c, d, e) ->
        p_byte 9 st
        p_tup5 p_dummy_range p_dtree p_targets p_dummy_range p_ty (a, b, c, d, e) st
    | Expr.Obj(_, b, c, d, e, f, g) ->
        p_byte 10 st
        p_tup6 p_ty (p_option p_Val) p_expr p_methods p_intfs p_dummy_range (b, c, d, e, f, g) st
    | Expr.StaticOptimization(a, b, c, d) ->
        p_byte 11 st
        p_tup4 p_constraints p_expr p_expr p_dummy_range (a, b, c, d) st
    | Expr.TyChoose(a, b, c) ->
        p_byte 12 st
        p_tup3 p_tyar_specs p_expr p_dummy_range (a, b, c) st
    | Expr.Quote(ast, _, _, m, ty) ->
        p_byte 13 st
        p_tup3 p_expr p_dummy_range p_ty (ast, m, ty) st
...
and p_static_optimization_constraint x st =
    ...
    | TTyconEqualsTycon(a, b) ->
        p_byte 0 st
        p_tup2 p_ty p_ty (a, b) st
    | TTyconIsStruct a ->
        p_byte 1 st
        p_ty a st

and p_slotparam (TSlotParam(a, b, c, d, e, f)) st =
    p_tup6 (p_option p_string) p_ty p_bool p_bool p_bool p_attribs (a, b, c, d, e, f) st

and p_slotsig (TSlotSig(a, b, c, d, e, f)) st =
    p_tup6 p_string p_ty p_tyar_specs p_tyar_specs (p_list (p_list p_slotparam)) (p_option p_ty) (a, b, c, d, e, f) st

and p_method (TObjExprMethod(a, b, c, d, e, f)) st =
    p_tup6 p_slotsig p_attribs p_tyar_specs (p_list p_Vals) p_expr p_dummy_range (a, b, c, d, e, f) st
...
and u_slotparam st =
    let a, b, c, d, e, f =
        u_tup6 (u_option u_string) u_ty u_bool u_bool u_bool u_attribs st

    TSlotParam(a, b, c, d, e, f)

and u_slotsig st =
    let a, b, c, d, e, f =
        u_tup6 u_string u_ty u_tyar_specs u_tyar_specs (u_list (u_list u_slotparam)) (u_option u_ty) st

    TSlotSig(a, b, c, d, e, f)

and u_method st =
    let a, b, c, d, e, f =
        u_tup6 u_slotsig u_attribs u_tyar_specs (u_list u_Vals) u_expr u_dummy_range st
...
let unpickleCcuInfo st =
    let a, b, c, _space =
        u_tup4 unpickleModuleOrNamespace u_string u_bool (u_space 3) st

    {
        mspec = a
        compileTimeWorkingDir = b
        usesQuotations = c
    }
