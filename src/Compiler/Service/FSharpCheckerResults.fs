...
type internal TypeCheckInfo
    ...
    member _.GetDeclarationListSymbols(parseResultsOpt, line, lineStr, partialName, isFromAttribute, getAllEntities, genBodyForOverriddenMeth) =
        let isSigFile = SourceFileImpl.IsSignatureFile mainInputFileName
    ...
    member _.ResolveNameAtLocation(cursorPos, plid: string list): FSharpSymbolUse option =
         ...
         match ResolveExprLongIdent TcResultsSink.NoSink ncenv m ad nenv TypeNameResolutionInfo.Default plid None with
         | Exception _ -> None 
         | Result (_, item, _) ->
...
type FSharpCheckFileResults
    ...
    member _.GetDeclarationListSymbols(parsedFileResults, line, lineText, partialName, isFromAttribute, ?getAllEntities, ?genBodyForOverriddenMeth) =
        let getAllEntities = defaultArg getAllEntities (fun () -> [])
        let genBodyForOverriddenMeth = defaultArg genBodyForOverriddenMeth true
        ...
        | None -> []
        | Some(scope, _builderOpt) ->
            scope.GetDeclarationListSymbols(parsedFileResults, line, lineText, partialName, isFromAttribute, getAllEntities, genBodyForOverriddenMeth)
